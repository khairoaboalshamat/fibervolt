...
export default function AddressLookup({ pins, clientMap }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        {suggestions.length > 0 && (
          <div className="absolute z-20 w-full mt-1 bg-card border border-border rounded-xl shadow-lg overflow-hidden">
            {suggestions.map(s => (
              <button key={s.place_id} onClick={() => selectAddress(s)} className="w-full text-left px-4 py-2.5 text-sm hover:bg-muted transition-colors border-b border-border last:border-0" style={{ color: '#111827' }}>
                {s.description}
              </button>
            ))}
          </div>
      ...
      {loading && <div className="flex items-center gap-2 text-sm" style={{ color: '#6b7280' }}><Loader2 className="h-4 w-4 animate-spin" /> Checking...</div>}
